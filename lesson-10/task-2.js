...
// Решение
const calculate = function(firstFn, ...functions) {
    if(typeof firstFn !== 'function'){
        throw new Error ('first arguments of function calculate() is not a function')
    }

    functions.map((fn) => {
        if(typeof fn !== 'function'){
            throw new Error ('rest arguments of function calculate() is not a function')
        }
    });

    const resultFirstFn = firstFn();

    return functions.reduce((acc, item) => {

        const foo = acc(resultFirstFn);
        return item(foo);
    });
};
